...
konc$ = UCASE$(RIGHT$(COMMAND$, 3)) 'BMP, JPG, PNG, GIF, PNM, XPM, XCF, PCX, TIF, LBM, and TGA
IF konc$ = "BMP" OR konc$ = "JPG" OR konc$ = "GIF" OR konc$ = "PNG" OR konc$ = "PNM" OR konc$ = "XPM" OR konc$ = "XCF" OR konc$ = "PCX" OR konc$ = "TIF" OR konc$ = "LBM" OR konc$ = "TGA" THEN


    j& = _NEWIMAGE(320, 240, 32)
    _SOURCE j&
    SCREEN j&
    PRINT "Konvertuji soubor do JPG, prosim cekejte..."
    LOCATE 2, 1: PRINT "Zdroj: "; COMMAND$; " na "; LEFT$(COMMAND$, LEN(COMMAND$) - 4) + ".JPG"
    _SOURCE 0

    i& = _LOADIMAGE(COMMAND$, 32)
    genx = _WIDTH(i&): geny = _HEIGHT(i&)
    _SOURCE i&

    GOTO dal

END IF
PRINT "Tato verze podporuje formaty BMP, JPG, PNG, GIF, PNM, XPM, XCF, PCX, TIF, LBM a TGA, kompresni pomer 0.05, vystupni rozliseni 1980 x 1050 px.  Program upravil Petr Preclik.": SLEEP 3: SYSTEM

dal:
REM ------ pod je original -------
REM OPEN "test.jpg" FOR OUTPUT AS #1
REM CLOSE
outfilename$ = LEFT$(COMMAND$, LEN(COMMAND$) - 4) + ".JPG"
OPEN outfilename$ FOR BINARY AS #1

'Set quality tables
'The smaller the paramter, the higher the quality
'0.01 is 100% quality
JPEG.StandardQT 0.01, QT()
